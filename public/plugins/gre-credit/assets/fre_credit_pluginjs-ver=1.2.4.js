(function($, Models, Collections, Views) {
	Views.freCreditSubmitProject = Views.Modal_Box.extend({
		...
		choosePlan : function ($step, $li, view){
			...
			if(parseFloat(price) > parseFloat(fre_credit_globals.available_of_user)){
				$('.gre-credit-payment-onsite').addClass('not-money');
				$('.gre-credit-payment-onsite a').addClass('btn-not-money');
				$('.gre-credit-payment-onsite a').removeClass('other-payment');
				$('.gre-credit-payment-onsite a').attr('href','');
				$('.gre-credit-payment-onsite .error').show();
			} else {
				$('.gre-credit-payment-onsite').removeClass('not-money');
				$('.gre-credit-payment-onsite a').removeClass('btn-not-money');
				$('.gre-credit-payment-onsite a').addClass('other-payment');
				$('.gre-credit-payment-onsite a').attr('href', '#gre-payment-frecredit');
				$('.gre-credit-payment-onsite .error').hide();
			}
	...
	Views.Modal_Withdraw = Views.Modal_Box.extend({
		...
		setupFields: function(){
			...
			$.ajax({
				...
				type: 'get',
				data: {
					action: 'gre-credit-get-balance-info'
				},
				beforeSend: function() {},
				success: function(res) {
					view.data = res;
		...
		initValidate: function(){
			...
			view.form_validator = view.$form.validate({
				...
				highlight:function(element, errorClass, validClass){
					...
					// $parent.addClass('has-error');
					// $target.addClass('has-visited');
					$(element).closest('.gre-input-field').addClass('error');
				...
				unhighlight:function(element, errorClass, validClass){
					...
					// var $container = $(element).closest('div');
					// $container.find('i.fa-exclamation-triangle').remove();
					$(element).closest('.gre-input-field').removeClass('error');
				}
			});
		},
	...
	Views.Modal_Edit_EmailCredit = Views.Modal_Box.extend({
		...
		setupFields: function(){
			...
			$.ajax({
				...
				type: 'get',
				data: {
					action: 'gre-credit-get-profile-info'
				},
				beforeSend: function() {},
				success: function(res) {
					var data = res.data;
		...
		sendRequest : function(e){
			...
			$.ajax({
				...
				type: 'get',
				data: {
					action: 'gre-credit-update-email-paypal',
	...
	Views.freCreditPage = Backbone.View.extend({
		...
		filterDate: function (name, value) {
			...
			historyCt.fetch($('#'+name).closest('#gre-credit-transaction'));
		},
        ...
        clearFilter :function(event){
            ...
            historyCt.fetch($target.closest('#gre-credit-transaction'));
        }
	});
	Views.creditBody = Backbone.View.extend({
		...
		requestSecureCode: function(e){
			...
			$target = $(e.currentTarget);
			var data = {
				action: 'gre-credit-request-secure-code'
			};
			$.ajax({
				url: ae_globals.ajaxURL,
				type: 'post',
	...
	$(document).ready(function() {
		...
		Models.history = Backbone.Model.extend({
			action: 'gre-history-sync',
			initialize: function() {}
		});
		Collections.history = Backbone.Collection.extend({
			model: Models.history,
			action: 'gre-fetch-history',
			initialize: function() {
				this.paged = 1;
			}
		});
		if($('#gre-credit-history-loop').length > 0){
			var historyItem = Views.PostItem.extend({
				tagName: 'div',
				className: 'gre-table-row',
				template: _.template($('#gre-credit-history-loop').html()),
			...
			ListHistory = Views.ListPost.extend({
				...
				itemClass: 'gre-table-row'
			});
			// notification list control
			if( $('.gre-credit-history-wrapper').length > 0 ){

				if( $('.gre-credit-history-wrapper').find('.fre_credit_history_data').length > 0 ){
					var postsdata = JSON.parse($('.gre-credit-history-wrapper').find('.fre_credit_history_data').html()),
				...
				new ListHistory({
					itemView: historyItem,
					collection: posts,
					el: $('.gre-credit-history-wrapper').find('.list-histories')
				});
				/**
				...
				Views.historyControl = Views.BlockControl.extend({
					events: function() {
						return _.extend({}, _.result(Views.BlockControl.prototype, 'events') || {}, {
							'change .gre-credit-history-filter-type': 'historyFilterType',
							'change .gre-credit-history-filter-status': 'historyFilterStatus'
						});
					},
                    historyFilterType: function(event){
						...
						if (name !== 'undefined') {
							...
							view.page = 1;
							// fetch page
							view.fetch($target.closest('#gre-credit-transaction'));
						}
					},
					...
					onBeforeFetch: function(){
	                    if($('.gre-credit-history-wrapper').find('.no-transaction').length > 0 ){
	                        $('.gre-credit-history-wrapper').find('.no-transaction').remove();
	                    }
	                },
	                onAfterFetch: function(result, res){
	                    if( !res.success || result.length == 0){
	                        //$('.gre-credit-history-wrapper').find('.list-histories').find('.gre-table-row').remove();
	                        //$('.gre-credit-history-wrapper').find('.list-histories').closest('div').append(fre_credit_globals.no_transaction_msg);
							$(fre_credit_globals.no_transaction_msg).insertAfter($('.gre-credit-history-wrapper').find('.list-histories'));
	                    }
                    }
				});
				historyCt = new Views.historyControl({
					collection: posts,
					el: $('.gre-credit-history-wrapper')
				});
