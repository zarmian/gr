(function($, Models, Collections, Views) {
    $(document).ready(function() {
		...
		$('.btn-gre-credit-payment, .btn-withdraw, .not-have-bid, .btn-submit-price-plan').popover({
			trigger: 'hover click',
			placement: 'auto',
		});
        ...
        /*$('.author_view_more').click(function () {
            ...
            var loading = $('<div class="loading-blur loading"><div class="loading-overlay"></div><div class="gre-loading-wrap"><div class="gre-loading"></div></div></div>');
                    ...
                    if(data.success == true){
                                ...
                                $( data.data ).each(function( k,v) {
                                	...
                                	if(v.tax_input.skill && v.tax_input.skill.length > 0){
                                        $( v.tax_input.skill ).each(function( ks,vs) {
                                            list_skills += '<span class="gre-label">'+vs.name +'</span>';
                                        });
									}
                        ...
                        var PortfolioView = Backbone.View.extend({
                            ...
                            events: {
                                ...
                                "click .gre-chosen-multi": "chosenClick"
                            },
		...
		*/
		Views.Front = Backbone.View.extend({
			...
			events: {
				...
				'submit .gre-gdpr-form-js': 'submitGDPR',
			},
		...
		*/
		...
		Views.EditProject =  Views.SubmitPost.extend({
		        onAfterInit: function() {
		            ...
		            view.$('.gre-calendar').datetimepicker({
						format: 'MM/DD/YYYY',
						icons: {
		            ...
		            if(!this.isMobile){
		            	$('.gre-chosen-category').chosen({
							width: '100%',
				            max_selected_options: parseInt(ae_globals.max_cat),
				            inherit_select_classes: true
						});
						$('.gre-chosen-skill').chosen({
							width: '100%',
						...
						$('.gre-chosen-category').change(function(event) {
						...
						$('.gre-chosen-skill').change(function(event) {
		        ...
		        reSetupSkills: function(){
					var view = this,
						form_field = view.$('.gre-input-field');
	    ...
	    */
		Views.SubmitProject = Views.SubmitPost.extend({
	        onAfterInit: function() {
            	...
            	if( view.$('ul.gre-post-package li input[type="radio"]:checked').length == 0){
            		view.$('ul.gre-post-package li').each(function(key, value){
						if(!$(value).hasClass('disabled')){
							$(value).find('input[type="radio"]').prop('checked', true);
							return false;
					    }
					})
            		// view.$('ul.gre-post-package li input[type="radio"]').first().prop("checked", true);
            	}
	        },
	        setupFields: function() {
	            var view = this,
		...
		$('.trigger-menu').click(function(){
			...
			$('body').addClass('gre-menu-overflow');
			// $('#video-background-wrapper').hide();
		});

		$('.overlay-close').click(function() {
		   $('body').removeClass('gre-menu-overflow');
