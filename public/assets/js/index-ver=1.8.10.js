(function ($, Models, Collections, Views) {
    $(document).ready(function () {
        ...
        $('.btn-gre-credit-payment, .not-have-bid, .btn-submit-price-plan').popover({
            trigger: 'hover',
            placement: 'auto',
        });
        ...
        $('.section-archive-project, .tab-project-home').each(function () {
            ...
            new Views.BlockControl({
                ...
                onAfterInit: function () {
                    ...
                    if (view.$el.find('#search_data').length > 0) {
                        ...
                        if (search_data.keyword) {
                            $('.gre-project-list-filter input[name="s"]').val(search_data.keyword).keyup();
                        }
                    }
                    ...
                    if (view.getUrlParameter('skill_project')) {
                        setTimeout(function () {
                            $('.gre-skill-dropdown li a[name="' + view.getUrlParameter('skill_project') + '"]').click();
                            $('body').click();
                        }, 1000);
                    }
                    // fitler category
                    if (view.getUrlParameter('category_project')) {
                        setTimeout(function () {
                            //$('#project_category.gre-chosen-single').val('website-project-management');
                            $('#project_category.gre-chosen-single').trigger("chosen:updated").change();

                        }, 1000);
                    }
                    // filter skill
                    $('.gre-skill-dropdown').on('click', function (event) {
                ...
                clearFilter: function (event) {
                    ...
                    view.$el.find('form ul.gre-skill-dropdown li .gre-skill-item').removeClass('active');
                    // reset query
                ...
                filterRanger: function () {
                    var view = this,
                        $et_budget = $('.gre-budget-field input[name="et_budget"]'),
                        name = $et_budget.attr('name'),
                        $min = $('.gre-budget-field input[name="min_budget"]'),
                        $max = $('.gre-budget-field input[name="max_budget"]');
                ...
                onBeforeFetch: function () {
                    ...
                    view.blockUi.block(view.$('.gre-project-list-box'));
                },
            ...
            new Views.BlockControl({
                ...
                onAfterInit: function () {
                    ...
                    if (view.$el.find('#search_data').length > 0) {
                        var search_data = JSON.parse(view.$el.find('#search_data').html());
                        if (search_data.keyword) {
                            $('.gre-profile-list-filter input[name="s"]').val(search_data.keyword).keyup();
                        }
                    }
                    // filter skill
                    if (view.getUrlParameter('skill_profile')) {
                        setTimeout(function () {
                            $('.gre-skill-dropdown li a[name="' + view.getUrlParameter('skill_profile') + '"]').click();
                            $('body').click();
                        }, 1000);
                    }
                    $('.gre-skill-dropdown').on('click', function (event) {
                ...
                filterRanger: function () {
                    var view = this,
                        $hour_rate = $('.gre-budget-field input[name="hour_rate"]'),
                ...
                onBeforeFetch: function () {
                    var view = this;
                    view.blockUi.unblock();
                    view.blockUi.block(view.$('.gre-profile-list-box'));
                },
                onAfterFetch: function (result, resp) {
                    var view = this;
                    view.addViewAllProfile();
