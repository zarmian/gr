(function($, Models, Collections, Views) {
    ...
    Views.Profile = Backbone.View.extend({
        ...
        events: {
            ...
            'click a.gre-view-portfolio-new': 'openModalViewPortfolio',
        ...
        initialize: function() {
            ...
            //button available
            $('#gre-switch-user-available').change(function () {
                var obj =$(this);
                if(obj.is(':checked')){
            ...
            $('select[name ="country"]').change(function () {
                var obj = $(this);
                if(obj.val()){
                    obj.closest('.gre-input-field').removeClass('error');
                }
            });
    ...
    Views.Modal_Add_Portfolio = Views.Modal_Box.extend({
                ...
                if(data){
                    ...
                    if(data.tax_input.skill && data.tax_input.skill.length > 0) {
                        $(data.tax_input.skill).each(function (ks, vs) {
                            if(vs.name){
                                html_skil += '<span class="gre-label">' + vs.name + '</span>';
                            }
                        })
                    }
        ...
        createPortfolio: function(event) {
            ...
            this.initValidator();
            var form = $(event.currentTarget),
                button = form.find('button.gre-submit-portfolio'),
                view = this;
            /**
             * scan all fields in form and set the value to model user
             */
                ...
                $.ajax({
                    ...
                    success: function (data, statusText, xhr) {
                        if (data.success) {
                            ...
                            if(last == 3){
                                $('.meta_history_item_'+id).closest('.gre-profile-box').find('.gre-empty-optional-profile').fadeIn();
                            }
